...
class Resources:
    def __init__(self, parent, trace = None):
        ...
        with Image.open("..\Icon\Wave icon (full circle, quarter size, transparent).png") as image:
            self.logo_circular_small_padded = ImageTk.PhotoImage(self.pr.f.pad_image_with_transparency(image = image.resize((85, 85)), pixels = self.pr.c.padding_medium, keep_size = False))

        with Image.open("..\Icon\settings_cog.png") as image:
            self.icon_settings_image = ImageTk.PhotoImage(self.pr.f.pad_image_with_transparency(image = image.resize((85, 85)), pixels = self.pr.c.padding_medium, keep_size = False))